...
import com.zj.boot_web.common.base.Constant;
...
/**
 * 文件操作工具类
 *TypesName(类名)：FileUtil
 *Description(描述)：TODO 
 * @author Adger
 * @date 2018年6月11日下午2:23:57
 *
 */
public class FileUtil {
	...
	// ResourceBundle.getBundle("config/constant");

	// public static String fileUploadPath =
	// bundle.getString("file-upload.dir");
	public static String fileUploadPath = Const.FILE_UPLOAD_DIR; // 文件上传目录
	//public static String fileUploadUrl = Const.FILE_UPLOAD_URL; // 文件上传url地址
	...
	public static void removeDocument(String fileName) {
		...
		if (fileName.lastIndexOf(Constant.FilePostFix.ZIP_FILE) > 0) {
			delDir(fileName.substring(0,
					fileName.lastIndexOf(Constant.FilePostFix.ZIP_FILE))
					+ "/");
		}
	...
	public static boolean fileToZip(String sourceFilePath, String zipFilePath,
		...
		if (sourceFile.exists() == false) {
			...
			try {
				...
				if (zipFile.exists()) {
					throw new Exception(zipFilePath + "目录下存在名字为:" + fileName
							+ Constant.FilePostFix.ZIP_FILE + "打包文件.");
				} else {
					...
					if (null == sourceFiles || sourceFiles.length < 1) {
						...
						for (int i = 0; i < sourceFiles.length; i++) {
							...
							bis = new BufferedInputStream(fis,
									Constant.BYTE_BUFFER
											* Constant.BUFFER_MULTIPLE);
							int read;
							while ((read = bis.read(bufs, 0,
									Constant.BYTE_BUFFER
											* Constant.BUFFER_MULTIPLE)) != -1) {
	...
	public static void savePic(InputStream inputStream, String fileName, String filePath) {
		...
		try {
			...
			os = new FileOutputStream(tempFile.getPath() + File.separator
					+ fileName);
			// 开始读取
	...
	public static String saveFile(InputStream file, int type, String name,
			String tag) throws Exception {
		String fileName = fileName(type, name, tag);
	...
	public static File createTemporaryFile(InputStream file, String name)
			throws Exception {
	...
	private static void copyFile(File newFile, FileInputStream file)
			throws Exception {
	...
	public static File getNewFile(String filePath, String fileName) throws IOException {
		if (ComUtil.isEmpty(filePath)) {
			filePath = fileUploadPath;
		}
	 ...
	 * @return
	 */
	private static String fileName(int type, String name, String tag) {
		...
		str.append(DateUtil.getYyyyMMdd());
		str.append("/");
	 ...
	 * @throws Exception
	 */
	public static boolean unZipFiles(String sourcePath, String targetPath)
			throws Exception {
		System.setProperty("sun.zip.encoding",
				System.getProperty("sun.jnu.encoding"));
		InputStream is = null;
	...
	public static int getFileType(String originalFilename) {
		String postFix = originalFilename.split("//.")[originalFilename
				.split("//.").length - 1];
	...
	public static String getCurrPath(Class<?> clazz) {
		return StringUtil.utf8Decoding(clazz.getResource("/").getPath()
	 ...
	 * @throws Exception
	 */
	public static void moveFile(File sourceFile, String target)
			throws Exception {
		if (!sourceFile.exists() || !sourceFile.isFile()) {
			return;
		}
		InputStream inputStream = null;
		File targetFile = new File(target + File.separator
				+ sourceFile.getName());
	...
	public static String readFileContent(File file, String fullFilePath)
			throws IOException {
		String returnStr = "";
	...
	public static String image2Base64String(InputStream content)
			throws Exception {
	...
	public static byte[] base64String2Image(String base64String)
			throws Exception {
	...
	public static void main(String[] args) throws Exception {
		...
		// File("D://123.png"))));
		System.out.println("data:image/png;base64,".replaceAll(
				"data:image/(jpg|png|jpeg);base64,", ""));
	}
