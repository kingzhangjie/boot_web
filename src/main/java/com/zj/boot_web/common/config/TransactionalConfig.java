 ...
 */
@Configuration
public class TransactionalConfig extends WebMvcConfigurerAdapter{
	...
	private static final String[] DEFAULT_REQUIRED_METHOD_RULE_TRANSACTION_ATTRIBUTES = {
			"add*", "save*", "insert*", "delete*", "update*", "edit*",
			"batch*", "create*", "remove*", "lock*", "record*", "touch*",
			"apply*", "modify*", "execute*", "merge*", "put*", "sync*",
			"copy*", "change*", "adjust*", "start*", "init*", "upload*",
			"batch*", "has*", "publish*", "import*", "export*", "disable*",
			"cancel*", "submit*", "send*", "process*", "complete*", "hide*",
			"top*", "reset*", "bind*", "handle*", "reSubmit*", "close*",
			"sign*" };
	/**
	 * 默认的只读事务
	 */
	private static final String[] DEFAULT_READ_ONLY_METHOD_RULE_TRANSACTION_ATTRIBUTES = {
			"get*", "count*", "find*", "query*", "select*", "list*", "search*",
			"is*", "verify*", "check*", "read*", "*" };
	/**
	 * 自定义事务 BeanName 拦截
	 */
	...
	public TransactionInterceptor customizeTransactionInterceptor(
		...
		for (String methodName : DEFAULT_READ_ONLY_METHOD_RULE_TRANSACTION_ATTRIBUTES) {
			transactionAttributeSource.addTransactionalMethod(methodName,
					readOnly);
		}
		// 默认的传播事务配置
	...
	public BeanNameAutoProxyCreator customizeTransactionBeanNameAutoProxyCreator() {
		...
		beanNameAutoProxyCreator
				.setInterceptorNames(CUSTOMIZE_TRANSACTION_INTERCEPTOR_NAME);
		// 默认
		for (String defaultTransactionBeanNameSuffix : DEFAULT_TRANSACTION_BEAN_NAMES) {
			beanNameAutoProxyCreator
					.setBeanNames(defaultTransactionBeanNameSuffix);
		}
		// 定制
	...
	private RuleBasedTransactionAttribute requiredTransactionRule() {
		...
		required.setRollbackRules(Collections
